	...
	, m_pImGuiCtx(nullptr)
{
	...
	//Allow the input layout and deny unnecessary access to certain pipeline stages
...
void GUI::Render(const std::shared_ptr<CommandList>& commandList, const RenderTarget& renderTarget)
{
	...
	//ImGui might not actually render anything, in which case any pending resource barriers in the commandList will not be flushed
	//(Since resource barriers are only flushed when a draw command is executed), in this case manually flushing the barriers will ensure they are properly flushed before exiting the function
	...
	for (int i = 0; i < drawData->CmdListsCount; ++i)
	{
		...
		for (int j = 0; j < drawList->CmdBuffer.size(); ++j)
		{
			...
			else
			{
				...
				if (scissorRect.right - scissorRect.left > 0.0f && scissorRect.bottom - scissorRect.top > 0.0f)
				{
