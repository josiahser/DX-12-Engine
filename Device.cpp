    ...
    : m_Adapter(adapter)
{
    ...
    {
        m_DescriptorAllocators[i] = std::make_unique<MakeDescriptorAllocator>(*this, static_cast<D3D12_DESCRIPTOR_HEAP_TYPE>(i));
    }
    ...
    //Check features
    {
        ...
        if (FAILED(m_d3d12Device->CheckFeatureSupport(D3D12_FEATURE_ROOT_SIGNATURE, &featureData, sizeof(D3D12_FEATURE_DATA_ROOT_SIGNATURE))))
        {
...
std::shared_ptr<ByteAddressBuffer> Device::CreateByteAddressBuffer(size_t bufferSize)
{
    ...
    std::shared_ptr<ByteAddressBuffer> buffer = std::make_shared<MakeByteAddressBuffer>(*this, CD3DX12_RESOURCE_DESC::Buffer(bufferSize, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS));
...
std::shared_ptr<StructuredBuffer> Device::CreateStructuredBuffer(ComPtr<ID3D12Resource> resource, size_t numElements, size_t elementSize)
{
...
std::shared_ptr<IndexBuffer> Device::CreateIndexBuffer(Microsoft::WRL::ComPtr<ID3D12Resource> resource, size_t numIndices, DXGI_FORMAT indexFormat)
{
...
std::shared_ptr<VertexBuffer> Device::CreateVertexBuffer(Microsoft::WRL::ComPtr<ID3D12Resource> resource, size_t numVertices, size_t vertexStride)
{
...
{
    std::shared_ptr<PipelineStateObject> pso = std::make_shared<MakePipelineStateObject>(*this, pipelineStateStreamDesc);
...
{
    std::shared_ptr<ConstantBufferView> cbv = std::make_shared<MakeConstantBufferView>(*this, constantBuffer, offset);
    return cbv;
}
...
{
    std::shared_ptr<ShaderResourceView> shaderView = std::make_shared<MakeShaderResourceView>(*this, resource, srv);
    return shaderView;
}

std::shared_ptr<UnorderedAccessView> Device::CreateUnorderedAccessView(const std::shared_ptr<Resource>& resource, const std::shared_ptr<Resource>& counterResource, const D3D12_UNORDERED_ACCESS_VIEW_DESC* uav)
{
    std::shared_ptr<UnorderedAccessView> unorderedView = std::make_shared<MakeUnorderedAccessView>(*this, resource, counterResource, uav);
    return unorderedView;
}
