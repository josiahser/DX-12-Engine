...
//Since the material properties are now in aligned memory, they must be deleted using the _aligned_free method
...
void Material::SetReflectance(const DirectX::XMFLOAT4& reflect)
{
    m_MaterialProperties->Reflectance = reflect;
}
...
{
    m_MaterialProperties->IndexOfRefraction = index;
}
...
void Material::SetBumpIntensity(float bump)
{
    m_MaterialProperties->BumpIntensity = bump;
}
...
void Material::SetMaterialProperties(const MaterialProperties& materialProps)
{
    *m_MaterialProperties = materialProps;
}
