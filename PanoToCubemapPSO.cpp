...
PanoToCubemapPSO::PanoToCubemapPSO(Device& device)
{
	...
	CD3DX12_STATIC_SAMPLER_DESC linearRepeatSampler(0, D3D12_FILTER_MIN_MAG_MIP_LINEAR, D3D12_TEXTURE_ADDRESS_MODE_WRAP, D3D12_TEXTURE_ADDRESS_MODE_WRAP, D3D12_TEXTURE_ADDRESS_MODE_WRAP);
	
	CD3DX12_VERSIONED_ROOT_SIGNATURE_DESC rootSignatureDesc(PanoToCubemapRS::NumRootParameters, rootParameters, 1, &linearRepeatSampler);

	m_RootSignature = device.CreateRootSignature(rootSignatureDesc.Desc_1_1);

	//Create the PSO for Generate Mips shader
	...
	//Create some default texture UAV's to pad any unused UAV's during mipmap generation
	...
	for (UINT i = 0; i < 5; ++i)
	{
		...
		uavDesc.Texture2DArray.ArraySize = 6; //since it's a cubemap
