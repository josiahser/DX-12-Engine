	...
	, m_Fullscreen(false)
{
	...
	//It is recommended to always allow tearing if tearing is available
	swapChainDesc.Flags = m_TearingSupported ? DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING : 0;
	swapChainDesc.Flags |= DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT;

	//Now create the swap chain
	ComPtr<IDXGISwapChain1> dxgiSwapChain1;
	ThrowIfFailed(dxgiFactory5->CreateSwapChainForHwnd(d3d12CommandQueue.Get(), m_hWnd, &swapChainDesc, nullptr, nullptr, &dxgiSwapChain1));
...
SwapChain::~SwapChain()
{
	//Close the swapchains waitable event handle?? don't think it's necessary
...
void SwapChain::Resize(uint32_t width, uint32_t height)
{
	if (m_Width != width || m_Height != height)
	{
		...
		ThrowIfFailed(m_dxgiSwapChain->ResizeBuffers(BufferCount, m_Width, m_Height, swapChainDesc.BufferDesc.Format, swapChainDesc.Flags));
...
void SwapChain::UpdateRenderTargetViews()
{
	for (UINT i = 0; i < BufferCount; ++i)
	{
		...
		//Set the names for the backbuffer textures, useful for debugging
